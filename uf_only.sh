#!/bin/bash
#TANGO installer for UBUNTU <= 18.04 by Ariane
#ArianeBlow@hotmail.com
#Should be compatible with Debian 9 / 10.
...
read -e -p "[?] Enter the Splunk Indexer to forward logs to: (example: <IP or URL where Cowrie's installed>:9997) " SPLUNK_INDEXER
...
if [ $(whoami) != "root" ]; then
	...
	print_good "We are root (not Groot...)."
fi
